...
fn _sign(digest: felt252, key_pair: KeyPair<felt252, felt252>) -> Signature {
    let (r, s): (felt252, felt252) = key_pair.sign(digest).unwrap();
    Signature { r, s }
}

fn _create_erc20_loan(setup: Setup) -> felt252 {
    ...
    simple_proposal.collateral_amount = E18;

    erc20_mint(setup.t20.contract_address, borrower(), E18);

    start_cheat_caller_address(setup.t20.contract_address, borrower());
...
fn _create_erc721_loan(setup: Setup) -> felt252 {
    let mut simple_proposal = setup.simple_proposal;
    ...
    erc721_mint(setup.t721.contract_address, borrower(), 42);

    start_cheat_caller_address(setup.t721.contract_address, borrower());
...
fn _create_erc1155_loan(setup: Setup) -> felt252 {
    _create_erc1155_loan_failing(setup, '')
}

fn _create_erc1155_loan_failing(setup: Setup, revert_data: felt252) -> felt252 {
    let mut simple_proposal = setup.simple_proposal;
    ...
    erc1155_mint(setup.t1155.contract_address, borrower(), 42, E18);

    start_cheat_caller_address(setup.t1155.contract_address, borrower());
    setup.t1155.set_approval_for_all(setup.loan.contract_address, true);

    _create_loan(setup, simple_proposal, revert_data)
}

fn _create_loan(setup: Setup, _proposal: Proposal, revert_data: felt252) -> felt252 {
    let signature = _sign(setup.proposal.get_proposal_hash(_proposal), setup.key_pair);

    erc20_mint(setup.credit.contract_address, lender(), 10 * E18);

    start_cheat_caller_address(setup.signer.contract_address, lender());
    setup.credit.approve(setup.loan_token.contract_address, 10 * E18);
    stop_cheat_caller_address(setup.signer.contract_address);
    ...
    let proposal_spec = ProposalSpec {
        ...
        proposal_data,
        proposal_inclusion_proof: array![0],
        signature
    };
    ...
    cheat_caller_address_global(setup.signer.contract_address);
...
fn _repay_loan(setup: Setup, loan_id: felt252,) {
    _repay_loan_failing(setup, loan_id, '')
}

fn _repay_loan_failing(setup: Setup, loan_id: felt252, revert_data: felt252) {
    erc20_mint(setup.credit.contract_address, borrower(), E18);

    start_cheat_caller_address(setup.credit.contract_address, borrower());
    setup.credit.approve(setup.loan_token.contract_address, E18);
    ...
    start_cheat_caller_address(setup.loan.contract_address, borrower());
...
fn erc1155_mint(erc1155: ContractAddress, receiver: ContractAddress, id: u256, amount: u256) {
    store(
        erc1155,
        map_entry_address(
            selector!("ERC1155_balances"), array![receiver.into(), id.try_into().unwrap()].span(),
        ),
        array![amount.try_into().unwrap()].span()
    );
}
